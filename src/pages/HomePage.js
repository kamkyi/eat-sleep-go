import { ArrowRight, Car, Check, Clock3, HeartHandshake, Home, KeyRound, MessageCircle, ShieldCheck, ShoppingBag, Sparkles, Store } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="section partner-band"><div className="container"><div className="partner-band__card"><div className="partner-band__content"><p className="eyebrow"><Store size={16} aria-hidden="true" /> {t('home.partnerEyebrow')}</p><h2>{t('home.partnerTitle')}</h2><p>{t('home.partnerText')}</p><div className="partner-band__actions"><PrimaryButton to="/partner">{t('home.partnerButton')} <ArrowRight size={18} aria-hidden="true" /></PrimaryButton><SecondaryButton to="/shop">{t('home.partnerLink')}</SecondaryButton></div></div><ul className="partner-band__types">{partnerTypes.map(({ id, icon: Icon, labelKey, textKey }) => <li key={id}><span><Icon size={20} aria-hidden="true" /></span><strong>{t(labelKey)}</strong><small>{t(textKey)}</small></li>)}</ul></div></div></section>
